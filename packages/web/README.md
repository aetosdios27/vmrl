...
2. Events identify receipt IDs. Every event is hydrated using `receipts(uint256)`; indexed repository hashes are never displayed as repository names. On public chains the page uses a single Multicall3 `eth_call` and falls back to bounded individual reads when Multicall3 is unavailable (the local demo). The table and receipt dialog show real repository names, tags, declared revision, recorded artifact digest, signer, and receipt timestamps. The dialog includes the actual event transaction hash and, on Base Sepolia, a link to that transaction.
3. Choose **Verify artifact** for the exact receipt you want to check. Use the labeled file picker or drop exactly one file. SHA-256 is computed locally in a Web Worker (main-thread fallback), so hashing does not block the UI. The whole file is still read into memory, so artifacts over 256 MB are refused with a suggestion to use the CLI. Choosing or clearing a file discards old results. Closing or switching receipts discards file and trust state and ignores unfinished hashing results.
...
Reads use the viem HTTP transport with three retries and a 300 ms delay. Relative timestamps update on a timer after mount rather than during render, so server and client markup stay consistent.

## Security headers

`next.config.ts` sets `Content-Security-Policy`, `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, and `Cross-Origin-Opener-Policy`. The CSP allows connections only to the configured RPC origin and `self`, permits workers and inline styles, and additionally allows `'unsafe-eval'` in development for tooling. Rebuild after changing `NEXT_PUBLIC_VMRL_RPC_URL` so the CSP `connect-src` matches.
